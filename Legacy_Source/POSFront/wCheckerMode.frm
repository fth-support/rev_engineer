...
Begin VB.Form wCheckerMode 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Checker Mode;klg_Title"
   ClientHeight    =   2445
   ClientLeft      =   1905
   ClientTop       =   5985
   ClientWidth     =   4890
   ControlBox      =   0   'False
   Icon            =   "wCheckerMode.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin XtremeSuiteControls.TabControl otcForm1 
      ...
      Begin VSFlex7Ctl.VSFlexGrid ogdForm 
         ...
         FormatString    =   $"wCheckerMode.frx":058A
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
   ...
   End
End
Attribute VB_Name = "wCheckerMode"
...
Private Sub W_GETxLoginUser()
'----------------------------------------------------------
'Call:
'Cmt : ดึง User ที่ทำการเปิดเครื่องครั้งแรกกลับคืนมา '*Eaw 56-26-09  CommSheet TK-ISS3000-361-FAPL-127.xls
'----------------------------------------------------------
Dim tSql As String
Dim tUsr As String
Dim orsTemp As ADODB.Recordset
On Error GoTo ErrHandle
    tUsr = SP_SQLtFunction("", "FTEmpCode", "TCNMTerminalMtn", " WHERE FTTmnNUm='" & tVB_CNTerminalNum & "'")
    If tUsr <> "" Then
        tSql = "SELECT FTEmpCode,FTEmpFName,FTEmpLName "
        tSql = tSql & vbCrLf & " FROM TCNMEmpMtn "
        tSql = tSql & vbCrLf & " WHERE FTEmpCode = '" & tUsr & "'"
        Call SP_SQLbExecute(tSql, orsTemp)
        If Not (orsTemp.BOF And orsTemp.EOF) Then
            tVB_CNUserCode = SP_FEDtChkString(orsTemp, "FTEmpCode")
            tVB_CNUserName = Trim(SP_FEDtChkString(orsTemp, "FTEmpFName") & " " & SP_FEDtChkString(orsTemp, "FTEmpLName"))
            wMain.ostForm.Panels(4).Text = "User : " & tVB_CNUserName
        End If
    End If
...
Dim tSmnID$
    With ogdForm
        .rows = 2
        .TextMatrix(0, 1) = Trim(SP_GETtFunMnuFromArray("186501", aVB_FunMnu, tSmnID))
        .TextMatrix(0, 2) = tSmnID
        .TextMatrix(1, 1) = Trim(SP_GETtFunMnuFromArray("186502", aVB_FunMnu, tSmnID))
...
Dim tSmnID As String
    ...
    Me.Caption = SP_GETtFunMnuFromArray("1865", aVB_FunMnu, tSmnID)
    tVB_CNMnuCurrent = tSmnID
...
Dim orsTemp As ADODB.Recordset
    ...
    With ogdForm
        Select Case .ValueMatrix(.Row, 2)
            Case 186501
                Call SaveSetting("AdaPosMall", "POSFront", "CheckerMode", "0")
                bVB_CheckerMode = True
            Case 186502
                Call SaveSetting("AdaPosMall", "POSFront", "CheckerMode", "1")
                bVB_CheckerMode = False
                Call W_GETxLoginUser '*Eaw 56-26-09  CommSheet TK-ISS3000-361-FAPL-127.xls
            Case Else
                Call SaveSetting("AdaPosMall", "POSFront", "CheckerMode", "1")
                bVB_CheckerMode = False
        End Select
    ...
    Unload Me
    If bVB_CheckerMode Then            '*Eaw 56-05-31 CommSheet TK-ISS3000-215-ST 00 00 01.xls
        '------------------------------------------------------------------
