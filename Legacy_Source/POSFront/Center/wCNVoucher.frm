...
Begin VB.Form wCNVoucher 
   ...
   Begin XtremeSuiteControls.TabControl otcForm1 
      ...
      Begin VSFlex7Ctl.VSFlexGrid ogdForm 
         ...
         FormatString    =   $"wCNVoucher.frx":058A
         ScrollTrack     =   0   'False
         ScrollBars      =   3
         ScrollTips      =   0   'False
         MergeCells      =   0
...
Private Sub Form_Load()
    ...
    ogdForm.Select 0, 0, 0, ogdForm.Cols - 1
    ...
    With ogdForm
            ...
            Select Case tTranType
                Case "1"
                    ...
                    tVB_Transaction = aVB_TransMode(14).ID
                    tVB_TransactionSub = tVB_Transaction
                    tVB_TransName = aVB_TransMode(14).Name
                    tVB_TransNameABB = aVB_NameAbb(tEN_AbbVoucher - 1).UsrLangPri & ";" & aVB_NameAbb(tEN_AbbVoucher - 1).UsrLangSec
                    tVB_CNTypePrinter = aVB_TransMode(14).PrintType
                    Call SP_INSxTransHD(tVB_CNTblHD)
                    bVB_TrnVoucher = True
                Case "2"
                    tVB_CNMnuCurrent = .TextMatrix(.Row, 2)
                    If Not SP_USRbMnuAccept(nEN_Usr2AthRead, tVB_CNMnuCurrent) Then Exit Sub
                    tVB_Transaction = aVB_TransMode(20).ID
                    tVB_TransactionSub = tVB_Transaction
                    tVB_TransName = aVB_TransMode(20).Name
                    tVB_TransNameABB = aVB_NameAbb(tEN_AbbCancelVoucher - 1).UsrLangPri & ";" & aVB_NameAbb(tEN_AbbCancelVoucher - 1).UsrLangSec
                    tVB_CNTypePrinter = aVB_TransMode(20).PrintType
                    Call SP_INSxTransHD(tVB_CNTblHD)
                    bVB_TrnVoucher = True
            End Select
            Unload Me
...
'--------------------------------------------------------
'Call:
'Cmt: ทำการแสดงรายการของ Voucher Create or Cancel Voucher
'-------------------------------------------------------
Dim nRow%
Dim tSmnID$
With ogdForm
    ...
    .Cols = 3
    .ColHidden(2) = True
    .TextMatrix(0, 1) = Trim(SP_GETtVocabularyTicketFromArray("170301", aVB_TransMnuAll, tSmnID)) '"Voucher Create"
    .TextMatrix(0, 2) = tSmnID
    .TextMatrix(1, 1) = Trim(SP_GETtVocabularyTicketFromArray("170302", aVB_TransMnuAll, tSmnID)) '"Cancel Voucher"
